...
resource "aws_s3_object" "well_known_openid_configuration" {
  bucket = module.s3_bucket.s3_bucket_id
  key    = ".well-known/openid-configuration/"  # Folder path (must include the trailing slash)
  acl    = "private"  # Optional: specify access control
...
module "cloudfront" {
  ...
  origin = {
    something = {
      ...
      origin_path = "/.well-known/openid-configuration"
